...
void deposit(musteri *sendOwner){
	{
		...
		if ((fp != 0) && (fpEx != 0))
		{
			...
			if ((ekBakiye-500)<0)
			{
				printf("Yatiracaginiz tutari giriniz (ek hesabinizda %d TL borc vardir): ", ekBakiye - 500);
			}
			else
			{
...
void record(musteri *sendOwner) {
	...
	if (fp!=0)
	{
		...
		do
		{
			printf("Dekontun baslangic tarihini gun.ay.yil seklinde giriniz: ");
